...
class Execution:
    ...
    def trade(self, symbol: str, X: pd.DataFrame | None = None, atr: float | None = None, auc_score: float | None = 0.5, total_open_risk: float = 0.0) -> OrderResult:
        ...
        direction = None
        if prob_up >= getattr(self.risk.cfg, "min_prob_long", 0.55):
            direction = "long"
        elif (1 - prob_up) >= getattr(self.risk.cfg, "min_prob_short", 0.55):
            direction = "short"
        ...
        pip_value = pip_size * getattr(symbol_info, "trade_contract_size", 1.0)
        ...
        request = {
            ...
            "sl": float(sl),
            "tp": float(tp),
            "deviation": max(10, int(2 * (float(tick.ask) - float(tick.bid)) / pip_size)) if pip_size else 10,
            "magic": 424242,
            "comment": "ml-bot",
    ...
    def manage_trades(self, symbol: str, atr: float):
        # Delegate to risk manager which implements BE/trailing logic
        try:
